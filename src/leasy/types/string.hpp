...
namespace leasy {
  ...
  class BasicString // TODO: Reflection exports: ADD BASE-CLASSES
      : public std::basic_string<C, Traits, Allocator>,
        public Iterable<size_t, C>,
        public SupportsSpan<C> {
  public:
    ...
    void forEach(
      const std::function<void(const size_t &, C &)> &callback
    ) override {
      for (size_t i = 0; i < string_type::size(); ++i) {
        callback(i, (*this)[i]);
      }
    ...
    Span<const C> asSpan() const override {
      if (this->empty()) return {};
      return Span<const C>{&(*this)[0], size()};
    }

    Span<C> asSpan() override {
      if (this->empty()) return {};
      return Span<C>{&(*this)[0], size()};
    }
    ...
    size_type begin() const override {
      return 0;
    }

    size_type end() const override {
      return size();
    }
